...
public class RegisterServiceImpl implements IRegisterService {
    ...
    public  int[] executeCount(List<Register> registerList,int any){
        ...
        for (Register register : registerList) {
            int week = TimeTool.getWeekOfYear(TimeTool.StrToDate(register.getStart_time()));
            if(week==any){
                if(studentDao.getWorkDetail(register.getStudentId())!=null){
                    if(studentDao.getWorkDetail(register.getStudentId())==0){
                        inCountSet.add(register.getStudentId());
                    }else{
                        outCountSet.add(register.getStudentId());
                    }
                }
            }
        }
